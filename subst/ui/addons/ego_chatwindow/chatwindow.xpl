...
local function init()
  ...
  -- UT Addition: Register Event for receiving Messages (based on SirNukes Monkey Patch)
  RegisterEvent("Chat_Window_API.Print", menu.AddConsoleMessage)
...
function menu.editboxSendMessage(_, text, textchanged, isconfirmed, wastableclick)
	...
	-- kuertee start: signal md // UT EDIT: Add terms functionality, and terms_numeric

	-- Space separate terms.
	local terms = menu.Split_String_Multi(text, " ")
  ...
  -- Go through separated terms and try to convert them to a number, and split up property chains, converting each value to a number if possible
...
-- Notes: 
-- Converting MD object to lua:
-- ConvertIDTo64Bit($mdobject$)
-- ConvertStringToLuaID(tostring(C.GetPlayerID()))
--
-- 
-- Converting Lua Object to MD:
-- ConvertStringTo64Bit(tostring(C.GetPlayerID()))

	-- Echo this back to ai/md scope.
	AddUITriggeredEvent("Chat_Window_API", "text_entered", {terms = terms, text = text, terms_converted=terms_converted})
...
-- UT EDIT: Function to convert string inputs to numbers - better move this to a lib file


function menu.convertStringToNumber(str)
    -- Check for hexadecimal
    if str:match("^%-?0[xX][%da-fA-F]+$") then
        return tonumber(str)
    end
    -- Check for octal
    if str:match("^%-?0[0-7]+$") then
        return tonumber(str, 8)
    end
    -- Check for decimal and scientific notation
...
function menu.Split_String(this_string, separator)
    ...
    local position = string.find(this_string, separator, 1, true) -- use plain find
    if position == nil then
        return this_string, nil -- no separator found
    end
...
function menu.Split_String_Multi(this_string, separator)
    local substrings = {}  -- Add local keyword to properly scope the variable
    ...
    while remainder do
        left, right = menu.Split_String(remainder, separator)
        table.insert(substrings, left)
        remainder = right
    end
    return substrings
end

function menu.AddConsoleMessage(_,text)
    local currentUserId = 1 -- Assuming this is how you get the current user id
    local groupid = nil -- Assuming this is not relevant for console messages
    local timestamp = C.GetCurrentUTCDataTime() -- Current timestamp


    -- Add the new message to the messages table
    table.insert(menu.messagetexts, {text = text, timestamp = timestamp, author = "CONSOLE", authorid = currentUserId})
    menu.displayChat()
end
...
function menu.onShowMenu()
	...
	menu.displayChat()
  -- UT EDIT: Feedback to re-print messages/restore console (More like a hack..)
	AddUITriggeredEvent("Chat_Window_API", "chatwindow_opened")
